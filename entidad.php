...
//        print_r($field);die();
        ...
        <div class="card">
            ...
            <div class="row">
                ...
                <div class="col-2">
                    ...
                    <?php
                    $mod = $_SESSION['SISTEMA_ ADMCAPTURA']['mod'];
                    if ($mod != 9 && $mod != 10 && $mod != 12 && $mod != 13) {
                        if (isset($strnuevo) && $strnuevo) {
                            ...
                            $strnuevo .= "<input type=\"text\" id=\"mod\" name=\"mod\" value=\"" . htmlspecialchars($_GET['mod'], ENT_QUOTES, 'UTF-8') . "\" hidden=\"true\">";
                            ...
                            $strnuevo .= "<button class=\"btn btn-primary btn4\" id=\"btn_nuevo_registro\" type=\"submit\">";
                            $strnuevo .= "<span class=\"ul-btn__icon\"><i class=\"i-Gear-2\"></i></span>";
                            $strnuevo .= "<span class=\"ul-btn__text\">" . $boton_texto . "</span></button>";
                            $strnuevo .= "</form>";
                            echo $strnuevo;
                        } else {
                            echo "";
                        ...
                        foreach ($resultado as $key => &$val) {
                            ...
                            if (isset($niveles_acceso) && count($niveles_acceso) > 0) {
                                ...
                                for ($puntero_nivel = 0; $puntero_nivel < count($niveles_acceso); $puntero_nivel++) {
                                    $nivel_enlaces .= "<a class=\"dropdown-item\" href='javascript:;' onclick=\"detalle('" . $val[$id_prin] . "','" . $id_prin . "','" . $tabla . "','" . $niveles_acceso[$puntero_nivel] . "','" . $NOMBRE_CARPETA_PRINCIPAL . "','" . $_GET['mod'] . "','" . $nombre_modulo_padre . "','" . $cve . "');\">
                            ...
                            echo '<tr>';
                                if ($streditar) {
                                    ...
                                    $stractualiza .= "<button class=\"btn btn-icon btn-flat-primary\" id=\"btn_actualiza_registro\" title =\"EVALUAR REGISTRO\" name=\"button\"><i class=\"ti ti-pencil me-1\"></i></button>";
                                    $stractualiza .= "</form>";
                                    $stractualiza .= '</div>';
                                } else {
                                    $stractualiza = "";
    ...
    <!------------------------------------------------------------------------------------------------------------------------------------------------->
    <script>
function detalle(valor, llave, tabla, archivo, carpeta, mod, padre, cve) {
    ...
    let cveEncoded = Base64.encode(cve);
    window.location.href = "../" + carpeta + "/index.php?mod=" + mod + "&pila=" + sbniv + "&tabs=" + sbniv2 + "&cve=" + cveEncoded;
}
