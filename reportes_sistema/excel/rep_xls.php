...
if (isset($_GET['chain6'])) {
    $query_principal .= "SELECT " . $campos . " FROM " . $str_union_tabla . " " . $tabla_join . " " . $strWhere . " " . $cadena_busqueda;
} else {
...
<body>
    <table class="tftable" border="1">
        ...
        foreach ($predatos as $key => &$val) {
            echo '<tr>';
            for ($i = 0; $i < count($vector_campos); $i++) {
                if (substr(strtoupper($vector_campos[$i]), 0, 4) == 'FILE') {
                    echo ' <td><img class="round" src="../../' . $val[$vector_campos[$i]] . '" alt="Perfil" width="62"></td>';
                } else {
