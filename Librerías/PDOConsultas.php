<?php
/**
 * PDOConsultas - Biblioteca de funciones de PHP para interactuar con bases de datos utilizando PDO
 * Archivo: PDOConsultas.php
 * Autor: CHRISTOPHER DELGADILLO
 * Versión: 2
 * Fecha de última actualización: 01/01/2023
 */
...
//    }
    ...
    private function connectMysql() {
        $dsn = "mysql:host={$this->dbHostName};dbname={$this->dbName}";

        try {
            $this->dbObj = new PDO($dsn, $this->dbUserName, $this->dbPassword);
            $this->connectionStatus = 1;
        } catch (PDOException $e) {
            $this->setErrors($e->getMessage());
        }
    ...
    private function getFetchType() {
        $fetchType = isset($this->fetchType) && !is_null($this->fetchType) ? strtoupper($this->fetchType) : 'ASSOC';
     ...
     *  Returns pagination
     */
    public function pagination($page = 1, $totalrecords, $limit = 10, $adjacents = 1, $url = "javascript:;") {
        $pagination = "";
        if ($totalrecords > 0) {
            if (!$limit)
                $limit = 15;
            ...
            if ($lastpage > 1) {
                ...
                //previous button
                if ($page > 1)
                    $pagination .= "<li><a class='pdomodel-page' 'href=\"$url\" data-page=" . $prev . ">« prev</a></li>";
                else
                    $pagination .= "<li class=\"disabled\"><a class='pdomodel-page' data-page=" . $prev . " 'href=\"$url\">« prev</a></li>";

                //pages	
                if ($lastpage < 7 + ($adjacents * 2)) { //not enough pages to bother breaking it up
                    for ($counter = 1; $counter <= $lastpage; $counter++) {
                        if ($counter == $page)
                            $pagination .= "<li class=\"active\"><a class='pdomodel-page' 'href=\"$url\" data-page=" . $counter . ">$counter</a></li>";
                        else
                            $pagination .= "<li><a class='pdomodel-page' 'href=\"$url\"  data-page=" . $counter . ">" . $counter . "</a></li>";
                    }
                    ...
                    if ($page < 1 + ($adjacents * 3)) {
                        for ($counter = 1; $counter < 4 + ($adjacents * 2); $counter++) {
                            if ($counter == $page)
                                $pagination .= "<li class=\"active\"><a class='pdomodel-page' data-page=" . $counter . "  href=\"$url\">$counter</a></li>";
                            else
                                $pagination .= "<li><a class='pdomodel-page' 'href=\"$url\"  data-page=" . $counter . ">" . $counter . "</a></li>";
                        }
                        $pagination .= "<li class=\"elipses\">...</li>";
                        $pagination .= "<li><a class='pdomodel-page' 'href=\"$url\"  data-page=" . $lpm1 . ">" . $lpm1 . "</a></li>";
                        $pagination .= "<li><a class='pdomodel-page' 'href=\"$url\"  data-page=" . $lastpage . ">" . $lastpage . "</a></li>";
                    }
                    //in middle; hide some front and some back
                    elseif ($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2)) {
                        $pagination .= "<li><a class='pdomodel-page' 'href=\"$url\"  data-page=\"1\">1</a></li>";
                        $pagination .= "<li><a class='pdomodel-page' 'href=\"$url\"  data-page=\"2\">2</a></li>";
                        $pagination .= "<li class=\"elipses\">...</li>";
                ...
                //next button
                if ($page < $counter - 1)
                    $pagination .= "<li><a class='pdomodel-page' ' href=\"$url\"  data-page=" . $next . ">next</a></li>";
                else
