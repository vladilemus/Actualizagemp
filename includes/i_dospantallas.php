...
if ($str_check) {
    ...
    if ($consulta->totalRows > 0) {
        ...
        $campo = array();
        $entidad = 'PETICIONES PENDIENTES';
        $btn_guardar='GUARDAR';
        $id_prin = 'cve_peticion';
        $strWhere = 'WHERE c.cve_estatus = 3';   
        $a_order = '';
        ...
        //$str_impresora = TRUE; 
        //$str_impresora_destino='fichas_reporte_sistema/rep_cat_peticiones.php?';
        ...
        $a_search_campo = array('clave_sp', 'nombre', 'pfolio');
        $a_search_etiqueta = array('CLAVE DE SERVIDOR PUBLICO', 'NOMBRE', 'FOLIO');
        $a_search_tipo = array('text','text','text');

        //SECCION DE LOS NIVELES DE ACCESO
        /******************DESCOMENTAR ESTA SECCION SI NECESITAN LOS ARCHIVOS PHP**************************/
        $niveles_acceso=array('i_dospantallas.php');
        $niveles_acceso_etiqueta=array('PERFILES');

        $tabla='det_peticiones a';
        $campos_join  = 'a.cve_peticion,a.clave_sp,a.nombre, a.rfc,a.codigo_post, b.curp,c.des_estatus, b.email';
        $tabla_join = 'INNER JOIN dt_sp_erroneos b ON a.clave_sp = b.clave_sp
        ...
        //$streliminar = TRUE;
        //$strnuevo = TRUE;
        //$str_javascript='<script src="metodos_javascript/js_cat_peticiones.js"></script>';
        ...
        **************************************************************************************************************************************/
        ...
        /******************************************************FIN DE LOS MODALS*****************************************************/
            ...
            switch ($_POST['opc']) {
                ...
                $field[]=array('cve_peticion','cve_peticion','VISTA','number','OBLIGATORIO','int', '', array(0, 12),'30','',array(),'','');    
                $field[]=array('clave_sp','CLAVE DE SERVIDOR PUBLICO','VISTA','number','OBLIGATORIO','int', '', array(0, 12),'30','',array(),'','');
                $field[]=array('nombre','NOMBRE','VISTA','text','OBLIGATORIO','bigint', '', array(0, 12),'30','',array(),'','');
                $field[]=array('curp','CURP','VISTA','text','OBLIGATORIO','bigint', '', array(0, 12),'30','',array(),'','');
                $field[]=array('rfc','RFC','VISTA','text','OBLIGATORIO','bigint', '', array(0, 12),'30','',array(),'','');
                $field[]=array('codigo_post','CODIGO POSTAL','VISTA','text','OBLIGATORIO','bigint', '', array(0, 12),'30','',array(),'','');
                $field[]=array('des_estatus','ESTATUS','VISTA','text','OBLIGATORIO','bigint', '', array(0, 12),'30','',array(),'','');
                $field[]=array('email','CORREO ELECTRONIO','VISTA','text','OBLIGATORIO','bigint', '', array(0, 12),'30','',array(),'','');
                ...
                $field[]=array('cve_peticion','cve_peticion','VISTA','number','OBLIGATORIO','int', '', array(0, 12),'30','',array(),'','');
                $field[]=array('cve_desc','cve_desc','VISTA','text','OBLIGATORIO','bigint', '', array(0, 12),'30','',array(),'','');
                $field[]=array('cve_doc','cve_doc','VISTA','number','OBLIGATORIO','int', '', array(0, 12),'30','',array(),'','');
                $field[]=array('pfolio','pfolio','VISTA','text','OBLIGATORIO','varchar', '', array(0, 12),'30','',array(),'','');
                $field[]=array('Observaciones','Observaciones','VISTA','text','OBLIGATORIO','varchar', '', array(0, 12),'30','',array(),'','');
                $field[]=array('pfecha','pfecha','VISTA','date','OBLIGATORIO','date', '', array(0, 12),'30','',array(),'','');
                    break;
                case 3:
                    /******************SELECTS(EJEMPLO)***********************************************/
                    //$field[] = array('cve_estado', 'cve_estado', 'VISTA', 'select', 'OBLIGATORIO', 'int',$vector,array(0, 12),'30','','');
        ...
        $str_entidad = "entidad_dospantallas.php";
